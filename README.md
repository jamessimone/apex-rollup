...
- `Group By Row Delimiter` (optional) - if set, is appended after each row in the group by rollup to delimit results. Very useful with a rich text field! Defaults to `<p>` for new line if not supplied (which is the platform default for Rich Text fields).
